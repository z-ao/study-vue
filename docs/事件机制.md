...
Vue框架也提供自定义事件和原生事件的实现。我们通过事例来了解它实现的机制。    

为了简单易懂，我们通过手写render函数实现事件过程。    
分别创建一个自定义事件和原生事件。   
...
```
上面的逻辑，其实把data.on赋值给listeners，然后data.nativeOn赋值给data.on。
...
```
把父级组件的listeners赋值给子组件options._parentListeners属性里。   
所以组件只有on属性的事件与子组件有关联。     
然后调用initEvent事件。
